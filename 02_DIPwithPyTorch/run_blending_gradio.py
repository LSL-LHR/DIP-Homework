...
def create_mask_from_points(points, img_h, img_w):
    ...
    if len(points) > 2:
        ...
        # 防止越界
        pts[:, 0, 0] = np.clip(pts[:, 0, 0], 0, img_w - 1)
        pts[:, 0, 1] = np.clip(pts[:, 0, 1], 0, img_h - 1)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    if foreground_img.shape != blended_img.shape:
        foreground_img = F.interpolate(foreground_img, size=blended_img.shape[2:], mode='bilinear', align_corners=False)
        foreground_mask = F.interpolate(foreground_mask, size=blended_img.shape[2:], mode='nearest')
    ...
    # 扩展 (out_channels, in_channels, H, W)
    ...
    #fg_laplacian_masked = fg_laplacian * foreground_mask
    ...
    fg_laplacian_masked = fg_laplacian * overlap_mask
    blended_laplacian_masked = blended_laplacian * overlap_mask
    # 计算L2 loss
    diff = fg_laplacian_masked - blended_laplacian_masked
    loss = (diff ** 2).sum() / (background_mask.sum() * 3 + 1e-8)
