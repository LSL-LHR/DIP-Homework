...
def read_images_text(path):
    ...
    sorted_images = dict(sorted(images.items(), key=lambda x: natural_sort_key(x[1]['name'])))
...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num = 3000):
        ...
        # Sample 3D points to a bounded number so pure PyTorch rasterization
        # stays tractable on CPU-only environments.
        if maximum_pts_num is not None and len(self.points3D_xyz) > maximum_pts_num:
