...
class GaussianRenderer(nn.Module):
    ...
    def compute_gaussian_values(
        ...
        power = -0.5 * torch.einsum('nhwi,nij,nhwj->nhw', dx, inv_covs, dx)
        normalizer = 1.0 / (2.0 * np.pi * torch.sqrt(det))
        gaussian = normalizer.view(N, 1, 1) * torch.exp(power.clamp(max=0.0)) ## (N, H, W)
